...
namespace MarsRover
{
    class Program
    {
        ...
        {
            Validations fileTest = new Validations();
            List<string> validateArgs = fileTest.ValidateFile(args);

            if (validateArgs.Count == 0)
            {
                ...
                Validations roverTest = new Validations();
                List<string> validationList = roverTest.RoverTest(roverList, height, width);
                ...
                {
                    foreach (string ValidateOutputRover in validationList)
                    {
                        Console.WriteLine(ValidateOutputRover);
                    }
                }
            } 
            else
            {
                foreach (string ValidateOutputList in validateArgs)
                {
                    Console.WriteLine(ValidateOutputList);
                }
            }
        }
    }
